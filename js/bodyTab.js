...
layui.define(["element","jquery"],function(exports){
        ...
        for(var i=0;i<data.length;i++){
            ...
            if(data[i].children != undefined && data[i].children.length > 0){
                ...
                for(var j=0;j<data[i].children.length;j++){
                    ...
                    if (data[i].children[j].spread || data[i].children[j].spread == undefined) {
                        ulHtml += '<li class="layui-nav-item nav-three layui-nav-itemed">';
                    } else {
                        ulHtml += '<li class="layui-nav-item nav-three">';
                    }

                    if(data[i].children[j].target == "_blank"){
                        ulHtml += '<a data-url="'+data[i].children[j].href+'" target="'+data[i].children[j].target+'" style="position:relative;">';
                    }else{
                    ...
                    ulHtml += '</a>';
                    if (data[i].children[j].children !=undefined && data[i].children[j].children.length > 0) {
                        ulHtml += '<dl class="layui-nav-child exclude">';
                            for(var k=0;k<data[i].children[j].children.length;k++) {
                                if(data[i].children[j].children[k].target == "_blank"){
                                    ulHtml += '<dd><a class="padd-left" data-url="'+data[i].children[j].children[k].href+'" target="'+data[i].children[j].children[k].target+'">';
                                }else{
                                    ulHtml += '<dd><a class="padd-left" data-url="'+data[i].children[j].children[k].href+'">';
                                }
	...
	Tab.prototype.render = function() {
        ...
        $(".nav-three").on("click",function () {
            if (this.className.indexOf("layui-nav-itemed") == -1) {
                this.children[1].classList.add("layui-hide");
            } else {
                $(".exclude").addClass("layui-hide");
                this.children[1].classList.remove("layui-hide");
            }
        })

		for (var i =0;i < $(".nav-three").length;i++) {

            if ($(".nav-three")[i].children.length == 2) {
                if ($(".nav-three")[i].className.indexOf("layui-nav-itemed") == -1) {
                    $(".nav-three")[i].children[1].classList.add("layui-hide");
                }
            }
        }
